...
public class CT_DigitalInput {
    ...
    /**
     * The sole reason for seperate interrupt "state" flags (m_handleInterrupts and
     * m_isInterruptLatched)
     * is so that there are seperate flags for simply handing the interrupt on
     * conditions and doing relatively
     * complex logic within this class. Having the same variable for EVERY method to
     * alter saw inconsistent and
     * incorrect results, leading to that single flag being altered at inconsistent
     * times when it probably should/shouldn't
     * be. Additionally, the m_isInterruptLatched is more a replacement for calling
     * enable/disableInterrupts,
     * while m_handleInterrupts is, as the name suggests, handling them not and
     * necessarily enabling/disabling them alltogether.
     * There might be a way to simplify the flags down to one, but for now
     * handleInterrupts will be with the
     * onlyHandleInterruptsWhen method and everything else will use
     * m_isInterruptLatched.
     */

    /**
     * Negate logic is useful if you want to reverse the output of getStatus(). A
     * use for setting this variable true
     * would be if you were using a light beam sensor and wanted to differentiate
     * between searching for something blocking
     ...
     * Used for sensors connected through DIO ports on the RoboRio. Limit Switches,
     * Light Beam sensors, etc.
     * 
     * @param pin         the channel the sensor is on.
     * @param negateLogic can negate the return value of getStatus() for this
     *                    instance.
     */
     ...
     * POLLING/NON-INTERRUPT METHOD
     * 
     * Checks to see if the digital IO is tripped depending on if it is acive high
     * or low.
     * If the sensor was tripped, the method either passed in to a constructor or
     * DigitalIO method setMethodToRun will run.
     * This method will not work if an interrupt has been enabled.
     * The given method will only run ONCE and will have to be reset by the
     * DigitalIO method setMethodToRun.
     ...
     * onlyHandleInterruptsWhen() method to only run the method when certain
     * conditions are met.
     * To use a command, for example use: "() -> new PrintCommand("Interrupt
     * Fired").schedule" for the runnable.
     * 
     * @param runnable               the runnable that will run when the interrupt
     *                               is fired.
    ...
    public void setInterrupt(Runnable runnable, boolean interruptOnRisingEdge, boolean interruptOnFallingEdge) {
        ...
        BiConsumer<Boolean, Boolean> callback = (risingEdge, fallingEdge) -> runInterruptMethod(runnable, risingEdge,
                fallingEdge);
        try (AsynchronousInterrupt interrupt = new AsynchronousInterrupt(m_digitalInput, callback)) {
     ...
     * @param interruptOnRisingEdge  fire interrupt on the rising edge.
     * @param interruptOnFallingEdge fire interrupt on the falling edge.
     * @param time                   time in seconds for the interrupt to be ignored
     *                               after the interrupt is activated.
     */
    public void setTimedInterrupt(Runnable runnable, boolean interruptOnRisingEdge, boolean interruptOnFallingEdge,
            double time) {
        BiConsumer<Boolean, Boolean> callback = (risingEdge, fallingEdge) -> runTimedInterruptMethod(runnable,
                risingEdge, fallingEdge, time);
        try (AsynchronousInterrupt interrupt = new AsynchronousInterrupt(m_digitalInput, callback)) {
            interrupt.setInterruptEdges(interruptOnRisingEdge, interruptOnFallingEdge);
    ...
    /**
     * This method is for if you want to ignore interrupts after a certain action as
     * taken place.
     * Use this method in a periodic to gain full effect as it relies on iteration
     * to count time.
     * Use this method in conjunction with ignoreInterruptsNow().
     * 
     * @param seconds time in seconds interrupts will be ignored after
     *                ignoreInterruptsNow() is called.
     */
    ...
    /**
     * This method works with the ignoreInterruptsFor() method, which should be
     * called in a periodic.
     * Call this method when you want interrupts to be ignored for the amount of
     * time passed into ignoreInterruptsFor()
     */
    ...
    /**
     * Setter method for the latch interrupt variable. Will decide if said interrupt
     * will be run or not.
     * 
     ...
     * Returned value is negated depending on the value of negateLogic passed into
     * the constructor.
     * If no negateLogic boolean was passed into the constructor, then the logic
     * will not be negated.
     * 
